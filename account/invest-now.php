<?php
	...
	if (isset($_POST['invest'])) {
		...
		// Corrected: use bound parameter instead of direct variable
		$row = $conn->prepare("SELECT * FROM investment_plans WHERE id = :id");
		...
		} elseif ($invest_amount < $min_invest) {
			$_SESSION['error'] = 'Please select a plan that corresponds with your investment amount';
		} elseif ($invest_amount > $max_invest) {
			$_SESSION['error'] = 'Please select a higher plan';
		} else {
			try {
				// Insert transaction record
				$bquery = $conn->prepare("INSERT INTO transaction (user_id, trans_date, type, amount, remark, balance) 
					VALUES (:user_id, :start_date, :type, :invest_amount, :remarks, :new_balance)");
				$bquery->execute([
					'user_id' => $user_id,
				...
				$stmt = $conn->prepare("INSERT INTO investment (invest_plan_id, user_id, capital, returns, current, start_date, end_date, status) 
					VALUES (:plan_id, :user_id, :invest_amount, :returns, :current, :start_date, :end_date, :status)");
				$stmt->execute([
					'plan_id' => $plan_id,
					'user_id' => $user_id,
					'invest_amount' => $invest_amount,
					'returns' => $returns,
					'current' => $invest_amount + 1,
					'start_date' => $start_date,
					'end_date' => $end_date,
					'status' => $status
				]);
				...
				$activity = $conn->prepare("INSERT INTO activity (user_id, message, category, date_sent) 
					VALUES (:user_id, :message, :category, :start_date)");
				$activity->execute([
					'user_id' => $user_id,
					'message' => $message,
					'category' => $category,
